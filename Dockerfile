...
COPY --chown=buildpiper:buildpiper print_table.py /usr/local/bin/print_table.py
COPY --chown=buildpiper:buildpiper BP-BASE-SHELL-STEPS /opt/buildpiper/shell-functions/
COPY --chown=buildpiper:buildpiper BP-BASE-SHELL-STEPS/data /opt/buildpiper/data
RUN sed -i 's/\r$//' build.sh && chmod +x build.sh && chmod +x /usr/local/bin/print_table.py
